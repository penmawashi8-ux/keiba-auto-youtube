...
> **ポイント**: 各ニュース記事のサムネイル（OG画像）を背景として自動使用するため、
> 手動で背景画像を用意する必要はありません。
...
背景画像は **自動で取得**されます。手動での準備は不要です。

### 取得の優先順位

1. **RSSフィードのメディア画像**（`media:content`、`enclosure` タグ）
2. **記事URLの OG画像**（`og:image` メタタグ）
3. **フォールバック**：競馬場グリーン（`#1a3a1a`）の単色背景

取得した画像は 1080×1920 にリサイズ・クロップされ、テキスト可読性のために
下部40%に半透明のオーバーレイが自動適用されます。
