        ...
        for key, model_name in key_model_pairs:
            ...
            try:
                ...
                if abstract_horse_pattern.search(script):
                    ...
                    return i, True
                # 開催日程だけで内容が薄いスクリプトをスキップ
                # 「〇〇が〇月〇日に行われました/開催されました」が全内容の場合
                thin_schedule_pattern = _re.compile(
                    r"^[^。]{0,50}(?:行われました|開催されました|実施されました)[^。]*。$"
                )
                if thin_schedule_pattern.match(script.strip()):
                    print(f"[{i}]  → 開催日程のみで内容が薄いためスキップ: {script[:60]}")
                    return i, True
