...
SYSTEM_PROMPT = (
    ...
    "- 固有名詞は省略せず、正確に伝えること\n"
    "- 馬名の直後に続く括弧書き（性別・年齢・所属・厩舎など）は絶対に含めないこと。例：「チャリングクロス（牡3、美浦・奥村武厩舎）」ではなく「チャリングクロス」とだけ書くこと\n\n"
    "【内容について】\n"
...
RESULTS_SYSTEM_PROMPT = (
    ...
    "- レース名・開催場・距離・馬場状態\n"
    "- 1着馬名・騎手名・調教師名（あれば馬番・人気）。馬名直後の括弧書き（性別・年齢・厩舎など）は不要\n"
    "- 2着・3着馬名（記事に書いてあれば）\n"
    "- タイム・レコードの有無\n"
    ...
    def generate_one(args):
        ...
        for key, model_name in key_model_pairs:
            ...
            try:
                ...
                script = _re.sub(r'[（(][牡牝セ騸]\d+[、，,][^）)]*?[）)]', '', script)
                # 空白の重複を整理
