...
SYSTEM_PROMPT = (
    ...
    "- LIVE配信・生中継の「放送開始時刻」「配信チャンネル」「発走時刻」の案内だけで、レース結果や出走馬・騎手情報が一切ない記事\n"
    "- 「〇〇競馬LIVE」「スポーツナビLIVE」「競馬速報LIVE」など、配信スケジュール・中継告知だけが主な内容の記事\n"
    "- 「〇〇が声明を発表しました」「〇〇がコメントを出しました」など、声明・コメントの存在を告知するだけで、その声明・コメントの具体的な内容が本文に一切書かれていない記事\n"
    "- 議員・政治家・団体が競馬に関して何らかの意見・要求・声明を出したという事実だけが書かれていて、競馬ファンにとって有益な具体的情報（馬名・レース・制度変更の内容など）が何もない記事\n\n"
    "【最重要：絶対に守るルール】\n"
