...
SYSTEM_PROMPT = (
    ...
    "- 【特に重要】タイトルや本文に「予想」「追い切り」「最終調整」「今日発走」「発走予定」「出走予定」「今週」「今後」などが含まれる未来のレース記事・予想記事では、「〜が好位から抜け出した」「〜が逃げ切った」「〜が制した」「〜が制覇しました」「〜が差し切った」「〜が勝利した」「〜が優勝しました」など、レース中の動きや結果を表す文を絶対に作らないこと。予定・展望・注目点のみを述べること\n"
...
RESULTS_SYSTEM_PROMPT = (
    ...
    "- 提供されたニュース本文に書かれていること「だけ」を話すこと\n"
    "- ニュース本文に書かれていない情報は1文字も追加しないこと（推測・補足・創作すべて禁止）\n"
    "- 【絶対禁止】予想記事・追い切り記事・登録記事でレース結果を作ること。「〜が制覇しました」「〜が勝利しました」「〜が優勝しました」など過去形の結果文は絶対に使わないこと\n"
    "- 「こんにちは」「みなさん」などの呼びかけ・挨拶は禁止\n"
    "- いきなりレース結果の核心から始めること\n"
        ...
        for key, model_name in key_model_pairs:
            ...
            try:
                ...
                future_race_keywords = _re.compile(
                    r"今日発走|本日発走|発走予定|出走予定|今日の(?:レース|競馬)|"
                    r"今週(?:の)?(?:レース|競馬|注目|日曜|土曜|月曜|火曜|水曜|木曜|金曜)|"
                    r"今後|展望|注目馬|出走登録|登録馬|今後の(?:レース|出走)|"
                    r"予想|プレビュー|追い切り|最終調整|前日|週末の(?:レース|競馬)"
                )
                fabricated_result_pattern = _re.compile(
                    r"抜け出した|抜け出し[。、]|逃げ切った|逃げ切り[。、]|差し切った|差し切り[。、]|"
                    r"押し切った|押し切り[。、]|突き抜けた|粘り切った|"
                    r"制覇した|制覇しました|制しました|"
                    r"勝利しました|優勝しました|快勝しました|連覇しました|"
                    r"初勝利を(?:挙げ|飾)[たり]|初重賞制覇|重賞初制覇|"
                    r"(?:が|は)制した|(?:が|は)勝利した|(?:が|は)優勝した|(?:が|は)快勝した|"
                    r"(?:が|は)連覇した|(?:が|は)勝ち切った|(?:が|は)勝ちました"
                )
