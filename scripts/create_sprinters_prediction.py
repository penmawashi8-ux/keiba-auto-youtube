...
# ──────────────────────────────────────────────────────────────────────────────
YOUTUBE_DESCRIPTION = """\
【スプリンターズS 2026 GⅠ】2026年9月27日(日) 中山競馬場・芝1200m
土曜の大雨で稍重～重馬場を想定した最終予想です。
キーワードは「速い流れを好位で追走して止まらない馬」。

■予想印
◎本命　11 ルガル…2024年、ハイペースを好位で粘り切った今年と同じ型のレースの勝ち馬
○対抗　5 サウンドモリアーナ…キーンランドCでGⅠ馬を撃破、3枠5番の好枠
▲単穴　15 フリッカージャブ…重馬場の前哨戦で大きく飛躍、雨は追い風
△押さえ　3 ママコチャ／4 ジューンブレア／8 エーティーマクフィ／14 ウインカーネリアン
☆3着穴　1 レッドモンレーヴ…高松宮記念2着の末脚
消し寄り　スターアニス／ピューロマジック

■想定する展開
逃げ・先行型のワールズエンド(6)、ブラックチャリス(12)、ウインカーネリアン(14)、ピューロマジック(16)が揃い、外の2頭が主張するため前半は速くなる想定。
近年はハイペースでも好位の馬が勝っており、稍重～重なら2018年のように前・内が残りやすい。

■買い目例
馬連：11－5, 15, 3, 4, 8, 14（6点）
3連複フォーメーション：11 → 5, 15 → 1, 3, 4, 5, 8, 14, 15（11点）

■当日の最終チェック
1. 馬場が良～稍重まで回復 → ◎をサウンドモリアーナに上げ、スターアニスを△に戻す
2. 午前の芝で外差しばかり届く → 内が荒れているサイン。フリッカージャブを◎、3着候補に差し馬を追加
3. 内の先行馬が残っている → この予想のままでOK

最後は当日の馬場を優先して、楽しんでください。
※予想は個人の見解です。馬券の購入は自己責任でお願いします。

#スプリンターズステークス #スプリンターズS #スプリンターズS2026 #競馬予想 #ルガル #サウンドモリアーナ #フリッカージャブ #G1予想 #中山競馬場 #競馬
"""
...
NEWS_ENTRY = {
    ...
    "youtube_title": (
        "【スプリンターズS2026】最終予想｜重馬場想定なら本命はルガル！"
        "買い目と当日の最終チェックまで公開【競馬予想】"
    ),
