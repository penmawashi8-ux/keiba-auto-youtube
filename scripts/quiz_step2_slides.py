...
def _draw_bg(ax, fig):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("bg", [BG_MID, BG, "#070d14"])
    ax.imshow(
        gradient,
        aspect="auto",
        extent=[0, 1, 0, 1],
    ...
    ax.add_patch(mpatches.FancyBboxPatch(
        ...
        facecolor=ACCENT2, edgecolor="none", alpha=0.08,
...
def make_question_slide(q: dict, out_path: Path, clue_header: str = "G1 勝利歴",
    ...
    ax.text(0.43, 0.858, "この馬は誰？", ha="center", va="center",
            color=ACCENT, fontsize=44, fontweight="bold")
    ax.plot([0.10, 0.65], [0.828, 0.828], color=ACCENT, linewidth=2.0, alpha=0.5)
    ax.plot([0.65, 0.74], [0.828, 0.828], color=ACCENT2, linewidth=2.0)
    ax.text(0.965, 0.854, f"Q{q['number']}", ha="right", va="center",
            color=ACCENT2, fontsize=20, fontweight="bold")
    ax.text(0.965, 0.836, "/ 5", ha="right", va="center",
    ...
    ax.add_patch(mpatches.FancyBboxPatch(
        ...
        facecolor=CLUE_BG, edgecolor="none",
        zorder=2,
    ))
    ax.add_patch(mpatches.FancyBboxPatch(
        (0.035, 0.465), 0.007, 0.365,
        boxstyle="square,pad=0",
        facecolor=ACCENT, edgecolor="none",
        zorder=3,
    ))
    _draw_section_line(ax, 0.825, x_left=0.050, x_right=0.955, zorder=3)
    header_label = f"{clue_header}（一部）" if is_partial else clue_header
    draw_clue_header(ax, 0.812, header_label)

    # 番号付き箇条書き（左列・右列、わずかにx位置をずらす）
    half = (len(clues) + 1) // 2
    col_left  = clues[:half]
    ...
    for i, clue in enumerate(col_left):
        y = base_y - i * row_h
        ax.text(0.058, y, f"{i+1}.", ha="left", va="center",
                color=ACCENT, fontsize=22, fontweight="bold", zorder=3)
        ax.text(0.088, y, clue, ha="left", va="center",
                color=WHITE, fontsize=30, zorder=3)
    for i, clue in enumerate(col_right):
        y = base_y - i * row_h
        num = half + i + 1
        ax.text(0.548, y, f"{num}.", ha="left", va="center",
                color=ACCENT, fontsize=22, fontweight="bold", zorder=3)
        ax.text(0.578, y, clue, ha="left", va="center",
                color=WHITE, fontsize=30, zorder=3)
...
HORSE_IMG_DIR = Path("assets/horses")


def _find_horse_image(horse_name: str) -> "Path | None":
    for ext in (".jpg", ".jpeg", ".png"):
        p = HORSE_IMG_DIR / f"{horse_name}{ext}"
        if p.exists():
            return p
    return None


def _overlay_horse_photo(slide_path: Path, img_path: Path) -> None:
    """ffmpeg で馬の写真をスライド右カラムにオーバーレイ（crop-to-fill）"""
    probe = subprocess.run(
        ["ffprobe", "-v", "error", "-select_streams", "v:0",
         "-show_entries", "stream=width,height", "-of", "csv=p=0", str(slide_path)],
        capture_output=True, text=True,
    )
    if probe.returncode != 0 or "," not in probe.stdout:
        print("  WARNING: ffprobe 失敗 → 写真オーバーレイスキップ")
        return
    try:
    ...
    # プレースホルダー領域の内側 (axes: x=0.560, y=0.092, w=0.410, h=0.800)
    px = round(W * 0.564)
    py = round(H * 0.108)   # image y=0 is top: 1 - (0.092+0.800) = 0.108
    pw = round(W * 0.400)
    ph = round(H * 0.784)

    tmp = slide_path.with_suffix(".tmp.png")
    cmd = [
        "ffmpeg", "-y",
        "-i", str(slide_path),
        "-i", str(img_path),
        ...
        (f"[1:v]scale={pw}:{ph}:force_original_aspect_ratio=increase,"
         f"crop={pw}:{ph}[photo];"
         f"[0:v][photo]overlay={px}:{py}"),
        str(tmp),
    ]
    result = subprocess.run(cmd, capture_output=True)
    if result.returncode == 0:
        tmp.rename(slide_path)
        print(f"  写真オーバーレイ完了: {img_path.name}")
    else:
        print(f"  WARNING: 写真オーバーレイ失敗: {result.stderr.decode()[:120]}")
        tmp.unlink(missing_ok=True)


def make_answer_slide(q: dict, out_path: Path, part_title: str = "",
                      horse_img_path: "Path | None" = None):
    correct_idx = q["correct_index"]
    choices = q["choices"]
    correct_label = CHOICE_LABELS[correct_idx]
    correct_name  = choices[correct_idx]

    use_photo = horse_img_path is not None and horse_img_path.exists()
    ...
    if use_photo:
        # ── 2カラムレイアウト（左: テキスト、右: 馬の写真） ────────────

        ax.text(0.270, 0.845, "正解は", ha="center", va="center",
                color=GRAY, fontsize=22)
        ax.text(0.270, 0.800, f"{correct_label}.  {correct_name}！", ha="center", va="center",
                color=CORRECT_BORDER, fontsize=36, fontweight="bold")
        ...
        for i, (bx, by, bw) in enumerate(photo_positions):
            if i < len(choices):
        ...
        ax.add_patch(mpatches.FancyBboxPatch(
            (0.030, 0.092), 0.522, 0.330,
            boxstyle="round,pad=0.015",
            facecolor=PANEL, edgecolor=ACCENT2, linewidth=1.2,
            zorder=2,
        ))
        ax.plot([0.045, 0.138], [0.400, 0.400], color=ACCENT2, linewidth=1.0, zorder=3)
        ax.text(0.175, 0.400, "解 説", ha="center", va="center",
                color=ACCENT, fontsize=22, fontweight="bold", zorder=3)
        ax.plot([0.212, 0.305], [0.400, 0.400], color=ACCENT2, linewidth=1.0, zorder=3)
        expl = q["display_explanation"]
        exp_fs = 26 if len(expl) <= 22 else (22 if len(expl) <= 27 else (20 if len(expl) <= 31 else 18))
        ax.text(0.291, 0.228, expl, ha="center", va="center",
                color=WHITE, fontsize=exp_fs, multialignment="center", zorder=3)

        # 写真プレースホルダー（右カラム・ffmpegで上書き）
        ax.add_patch(mpatches.FancyBboxPatch(
            (0.560, 0.092), 0.410, 0.800,
            boxstyle="round,pad=0.015",
            facecolor=CLUE_BG, edgecolor=ACCENT2, linewidth=1.5,
            zorder=2,
        ))
    ...
    if use_photo:
        _overlay_horse_photo(out_path, horse_img_path)
...
def make_title_slide(title: str, out_path: Path, total_q: int = 5, multipart: bool = False):
    ...
    tri = mpatches.Polygon(
        [[0.82, 1.0], [1.0, 1.0], [1.0, 0.68]],
        facecolor=ACCENT2, edgecolor="none", alpha=0.30,
        transform=ax.transAxes, zorder=2,
    )
    ax.add_patch(tri)

    # メインパネル（左マージン広め・右マージン狭め）
    ax.add_patch(mpatches.FancyBboxPatch(
        (0.065, 0.195), 0.870, 0.630,
    ...
    if quiz.get("multipart"):
        ...
        for part in parts:
            ...
            for q in questions:
                ...
                correct_name = q["choices"][q["correct_index"]]
                make_answer_slide(q, OUTPUT_DIR / f"p{pn:02d}_{q['number']:02d}a_answer.png",
                                  part_title=pt,
                                  horse_img_path=_find_horse_image(correct_name))
                slide_count += 2
